...
```
L1 主 plan       ~/.claude/plans/validated-meandering-conway.md
                 ✅ 始终读(≤ 1000 行 · 30 秒速看)
                 § 0.1 速查 PR 表 / § 0.5 当前推进路径

L2 归档主体     ~/.claude/plans/archive/{date}-{slug}.md
                 ❌ 不默认 · 仅 NZH 问"v{N} 当时为啥"才读

L3 分支深历史   ~/.claude/plans/archive/branches-history/v{N}-r{X}-{date}-{slug}.md
                 ❌ 不默认 · 仅 NZH 问"v{N}.r{X} 增/删了啥"才读
```

**铁律**(整改协议 skill §15):任何 session 启动 · 仅读 L1 · L2/L3 0 默认载入 · 节省 95% cache。

**健康检查**:`bash ~/Project/ExamBoard/25maths-planning/scripts/plan-health-check.sh`
